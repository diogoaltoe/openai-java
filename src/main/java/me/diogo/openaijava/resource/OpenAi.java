...
public class OpenAi {
    ...
    public List<ChatCompletionChoice> chatRequest(final List<ChatMessage> chatMessages) {
        ...
        return doRequest(service, request, 0, 5);
    }
    ...
    private static List<ChatCompletionChoice> doRequest(OpenAiService service, ChatCompletionRequest request, int attempt, int waitInSeconds) {
        if(attempt > 4)
        ...
        try {
            ...
            switch (e.statusCode) {
                ...
                case 429 -> {
                    return prepareNewAttempt(service, request, attempt, waitInSeconds, "Rate limit reached.");
                }
                case 500, 503 -> {
                    return prepareNewAttempt(service, request, attempt, waitInSeconds, "API is not responding.");
                }
    ...
    private static List<ChatCompletionChoice> prepareNewAttempt(final OpenAiService service, final ChatCompletionRequest request, final int attempt, final int waitInSeconds, final String warningMessage) {
        ...
        return doRequest(service, request, attempt+1,waitInSeconds*2);
    }
}
